...
VIDEO_FPS = 30
W, H = 960, 720                  # showcase renders at a higher resolution than review
...
def render(rt: G1MujocoRuntime, P, Q, J, joint_names, fps):
    ...
    # The MJCF ships a 640x480 offscreen buffer and mujoco.Renderer refuses anything
    # larger. Raise it on the compiled model so the showcase can render above review
    # resolution; fall back rather than fail if this build disallows the write.
    global W, H
    try:
        if rt.model.vis.global_.offwidth < W:
            rt.model.vis.global_.offwidth = W
        if rt.model.vis.global_.offheight < H:
            rt.model.vis.global_.offheight = H
    except Exception:
        pass
    if rt.model.vis.global_.offwidth < W or rt.model.vis.global_.offheight < H:
        W, H = int(rt.model.vis.global_.offwidth), int(rt.model.vis.global_.offheight)
        print(f"   [warn] offscreen buffer capped, rendering at {W}x{H}")
    if getattr(rt, "_renderer", None) is not None:
        rt._renderer = None                 # size changed: drop the cached renderer
